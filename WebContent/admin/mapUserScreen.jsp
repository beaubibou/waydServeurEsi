...
<body>
	...
	<%=profil.getLongitudeFixe()%>
			...
			var_map.addListener('center_changed', function() {
				...
				if (encours == 0) {
					...
					window.setTimeout(function() {
						...
						$.get("MapUserList?latitude=" + lat + "&longitude="
		...
		function afficheMarker(reponseJson) {
			...
			$.each(reponseJson, function(index, user) { // Iterate over the JSON array.

				positionItem = new google.maps.LatLng(user.latitude,
						user.longitude);
				...
				var lien = "DetailParticipant?idPersonne=" + user.id;
				var contentString = getContent(user.pseudo, user.libelle, lien,
						"", user.photo);
	...
	<script
		src="https://maps.googleapis.com/maps/api/js?key=<%=Outils.getCleMap()%>&callback=init_map"
		async defer></script>

	<script>
		function getContent(titre, contenu, lien, cree, photo) {
			//alert(titre+ " "+photo);
...
// 					+ titre
// 					+ '</h1>'
// 					+ "<img height='80' width='80' src="+ "'"+photo+"'"+ " class='img-thumbnail pull-left'>"
// 					+ '<p><a href='+lien+'>' + 'Consulter</a> ' + '</div>'
// 					+ '</div>';

		
			var contentString ="<div class='clearfix'><img height='120' width='120' src=data:image/jpeg;base64,"+photo+" class='img-thumbnail pull-left'>"+
			"<a href="+lien+" <p>"+titre+"</p></a></div>";
