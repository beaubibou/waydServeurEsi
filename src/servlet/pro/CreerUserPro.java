...
public class CreerUserPro extends HttpServlet {
	...
	//public static boolean compteDesactiveParDefaut=true;
	...
	protected void doPost(HttpServletRequest request,
		...
		MessageServeur messageServeurTestRequete = testParametreRequete(pwd,
				pwd1, email, pseudo, siret, telephone, adresse, commentaire,
				latitude, longitude);

		if (!messageServeurTestRequete.isReponse()) {

			redirige(pwd, pwd1, email, pseudo, siret, telephone, adresse,
					commentaire, latitude, longitude, request, response,
					messageServeurTestRequete.getMessage());
		...
		MessageServeur messageServeurFireBase = creerUtilisateurFireBase(email,
				pwd, pseudo);

		if (messageServeurFireBase.isReponse()) {
			String uuid = messageServeurFireBase.getMessage();
			MessageServeur messageCreerUserDAO = creerUtilisateurDAO(uuid, pwd,
					pwd1, email, pseudo, siret, telephone, adresse,
					commentaire, siteweb, latitude, longitude);
			...
			if (messageCreerUserDAO.isReponse()) {
				...
				redirige(pwd, pwd1, email, pseudo, siret, telephone, adresse,
						commentaire, latitude, longitude, request, response,
						messageCreerUserDAO.getMessage());
			...
			redirige(pwd, pwd1, email, pseudo, siret, telephone, adresse,
					commentaire, latitude, longitude, request, response,
					messageServeurFireBase.getMessage());
	...
	private void redirige(String pwd, String pwd1, String email, String pseudo,
			String siret, String telephone, String adresse, String commentaire,
			double latitude, double longitude, HttpServletRequest request,
			HttpServletResponse response, String message) {
		request.setAttribute("alerte", new MessageAlertDialog(
				"Message Information", message, null, AlertJsp.warning));
		request.setAttribute("pwd", pwd);
	...
	private MessageServeur creerUtilisateurDAO(String uuid, String pwd,
		...
		try {
			...
			wayde.dao.PersonneDAO personnedao = new wayde.dao.PersonneDAO(
					connexion);
			personnedao.addComptePro(uuid, pseudo, email, adresse, siret,
					telephone, commentaire, siteweb, latitude, longitude);
			connexion.commit();
